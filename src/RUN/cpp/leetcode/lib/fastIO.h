    ...
    struct InputHelper
    {
        ...
        InputHelper &fillInteger(_Tp &__ret, _BinaryOperation __op)
        {
            ...
            while (next(), isdigit(getChar_Unchecked()))
                __ret = __op(__ret * 10, getChar_Unchecked() - '0');
            return *this;
        }
        ...
        InputHelper &operator>>(_Tp &__num)
        {
            ...
            if (isdigit(getChar_Unchecked()))
            {
                ...
                while (next(), isdigit(getChar_Unchecked()))
                    __num = __num * 10 + (getChar_Unchecked() - '0');
            }
            if (getChar_Unchecked() == '.')
                if (next(), isdigit(getChar_Unchecked()))
                {
                    ...
                    while (next(), isdigit(getChar_Unchecked()))
                        __num += (unit *= 0.1) * (getChar_Unchecked() - '0');
                }
            if (!integer && !decimal)
                return setBad();
            if (neg)
