...
int tCompare(string a, string b) //高精比较 设定a>b返回1 a<b返回-1 a=b返回0
{
...
string tadd(string a, string b) //高精加法
{
...
string tSubtract(string a, string b) //高精减法
{
    string c = "";           //创建一个字符串去存储答案
...
string tMultiply(string a, string b) //高精乘法
{
...
vector<string> tDivide(string a, string b) //高精除法
{
...
string tFactorial(string a) //高精阶乘
{                            /*我们还可以利用高精度减法和乘法实现大数的阶乘(最大可运行出10000左右的阶乘)*/
    if (a == "1")
        return a;
    else
        return tMultiply(a, tFactorial(tSubtract(a, "1")));
}
